    ...
    uint16_t u16Col)
{
  ...
  uint16_t u16NumberOfRectangle = u16Row * u16Col;
  if (u16NumberOfRectangle > 255)
  { // DWIN max items per command
    ESP_LOGE("xDrawBolderMatrixRectangle", "Number of rectangles (%u) exceeds 255 limit for single command.", u16NumberOfRectangle);
    return ESP_ERR_INVALID_ARG;
  }

  uint16_t u16GraphicDrawCMD = 0x0003;         // Lệnh vẽ hình chữ nhật (outline)
  const uint16_t u16BytePerRectangleData = 10; // Xs(2B), Ys(2B), Xe(2B), Ye(2B), Color(2B) = 10 bytes
  ...
  // --- Bắt đầu xây dựng toàn bộ lệnh DWIN để in ra/gửi đi ---
  uint8_t final_dwin_command[3 + u16PayloadSize];
  uint16_t final_idx = 0;

  final_dwin_command[final_idx++] = 0x5A;
  final_dwin_command[final_idx++] = 0xA5;
  final_dwin_command[final_idx++] = (uint8_t)u16PayloadSize; // Length byte

  memcpy(&final_dwin_command[final_idx], u8RectanglePayload, u16PayloadSize);
  final_idx += u16PayloadSize;

  printf("Generated DWIN Bolder Rectangle Command (Total %u bytes):\n", final_idx);
  for (uint16_t i = 0; i < final_idx; i++)
  {
    printf("%02X ", final_dwin_command[i]);
  }
  printf("\n");

  // return _dwin.sendArray(final_dwin_command, final_idx);
  return ESP_OK;
}
    ...
    uint16_t u16Col)
{
  ...
  uint16_t u16GraphicDrawCMD = 0x0004;         // Lệnh vẽ hình chữ nhật (fill)
  const uint16_t u16BytePerRectangleData = 10; // Xs(2B), Ys(2B), Xe(2B), Ye(2B), Color(2B) = 10 bytes
  uint16_t u16TotalByteRectangleData = u16NumberOfRectangle * u16BytePerRectangleData;
